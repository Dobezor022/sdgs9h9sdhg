...
usage() {
  cat <<'USAGE'
FedMes 2.0.1 Rust production installer

Required:
  export FEDMES_DOMAIN='fedmes.example.com'

Usage:
  sudo -E ./clean-install-2.0.1.sh [--preserve-data|--wipe-data]
...
required=(
  server-source.tar.gz build-rust-server-on-ubuntu.sh
...
STAGE="$(mktemp -d /tmp/fedmes-2.0.1-stage.XXXXXX)"
echo "[2/13] Rust compile + tests BEFORE downtime"
"$BUNDLE_DIR/build-rust-server-on-ubuntu.sh" "$BUNDLE_DIR/server-source.tar.gz" "$STAGE/rust-bin"
for binary in fedmes-server fedmes-qr fedmes-update-server fedmes-maintainer; do
  "$STAGE/rust-bin/$binary" --version 2>&1 | grep -Fq '2.0.1' || { echo "Неверная версия $binary" >&2; exit 1; }
done
...
BACKUP="/root/fedmes-before-2.0.1-$STAMP"
mkdir -p "$BACKUP"
backup_path() {
  local source="$1"
  [[ -e "$source" ]] || return 0
...
[Unit]
Description=FedMes Rust family messenger server 2.0.1
...
[Unit]
Description=FedMes Rust authenticated update service 2.0.1
...
[Unit]
Description=FedMes Rust maintainer 2.0.1
...
assert v.get('version')=='2.0.1',v
assert int(v.get('build'))==20001,v
PY
...
import json,sys
v=json.load(open(sys.argv[1],encoding='utf-8'))
assert v.get('server')=='rust' and v.get('version')=='2.0.1',v
PY
...
echo "Legacy OPAQUE/password endpoints are intentionally fail-closed in 2.0.1. Supported production admission: QR/device identity/recovery."

echo "[13/13] FedMes 2.0.1 Rust RELEASE installed"
echo "Backup: $BACKUP"

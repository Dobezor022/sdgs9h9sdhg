...
[[ -n "$DOMAIN" ]] || { echo "FEDMES_DOMAIN обязателен для production установки" >&2; exit 2; }
WIPE_DATA=0
for arg in "$@"; do
  case "$arg" in
...
[[ "$($BUNDLE_DIR/fedmes-qr --version 2>/dev/null | head -n1)" == "2.0.0" ]] || { echo "fedmes-qr в комплекте не версии 2.0.0" >&2; exit 1; }
[[ "$($BUNDLE_DIR/fedmes-update-server --version 2>/dev/null | head -n1)" == "2.0.0" ]] || { echo "fedmes-update-server в комплекте не версии 2.0.0" >&2; exit 1; }
$BUNDLE_DIR/fedmes-maintainer --version 2>&1 | grep -Fq 'FedMes Maintainer 2.0.0 (20000)' || { echo "fedmes-maintainer в комплекте не версии 2.0.0 build 20000" >&2; exit 1; }

STAMP="$(date +%Y%m%d-%H%M%S)"
BACKUP="/root/fedmes-before-clean-2.0.0-$STAMP"
mkdir -p "$BACKUP"
backup_path() {
  local source="$1"
  [[ -e "$source" ]] || return 0
...
echo "[4/10] Сборка сервера 2.0.0"
...
[Unit]
Description=FedMes family messenger server 2.0.0
...
[Unit]
Description=FedMes authenticated client update service 2.0.0
...
[Unit]
Description=FedMes automatic repair and server updater 2.0.0
...
User=root
Group=root
ExecStart=/opt/fedmes/bin/fedmes-maintainer --interval 1m --update-interval 30m --auto-server-update=true --server-update-url https://$DOMAIN/server-updates/latest.json
Restart=always
RestartSec=10s
UMask=0077
PrivateTmp=true
...
echo "[10/10] FedMes 2.0.0 установлен с нуля"
